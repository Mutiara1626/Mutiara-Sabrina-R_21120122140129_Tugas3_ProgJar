...
int main() {
    ...
    server_addr.sin_port = htons(1616);     // Menentukan port (4444)
    server_addr.sin_addr.s_addr = htonl(INADDR_ANY); // Mengikuti alamat IP local
    ...
    while (1) {
        ...
        while(1) {
            ...
            // Memeriksa apakah pesan adalah "babay" untuk keluar
            if (strcmp(msg, "babay") == 0) {
                printf("exiting...\n");
                break;
            }
        }
